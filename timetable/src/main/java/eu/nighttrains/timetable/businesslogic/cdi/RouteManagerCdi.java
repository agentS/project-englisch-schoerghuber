...
public class RouteManagerCdi implements RouteManager {
    ...
    public List<RailwayStationConnectionDto> findAllStopsBetween(Long originId, Long destinationId) throws NoRouteException {
        ...
        // TODO: Clean up this fix, as it is not rally clean
        RailwayStationConnectionSearchGraph searchGraph;
        try{
            searchGraph = RailwayStationConnectionSearchGraph.createFromConnections(allConnections);
        }catch (Exception exception){
            throw new NoRouteException("No route exists between train stations with IDs " + originId + " and " + destinationId + '.');
        }

        var searchPath = SearchGraphAlgorithms.findPathIterativeDeepeningSearch(searchGraph
                , destinationId
        );
        if (searchPath != null) {
